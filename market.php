...
postHead('');
AuthUser();
if ($CFU_Time >= $_SESSION['timeauth']+$TIME_OUT_TIME && $_SESSION['username'] != 'c0re' || $_SESSION['timeauth'] <= $CFU_Time-$TIME_OUT_TIME && $_SESSION['username'] != 'c0re'){echo "連線超時！<br>請重新登入！";exit;}
...
elseif ($actionb=='remit'){
		...
		if ($Gen['cash'] < $buy['price'] && $buy['owner']!="$_SESSION[username]"){echo "你的現金不足呢！";postFooter();exit;}
