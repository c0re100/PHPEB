...
echo "<p align=center style=\"font-size: 16pt\">置放完成了！<br><input type=submit value=\"返回\" onClick=\"parent.$SecTarget.location.replace('gen_info.php')\"><input type=button value=\"繼續\" onClick=\"freect.submit()\"></p>";
...
echo "</td><td><b>改造成功率: </b><span id=successpc>100</span>% (武器複雜度: <span id='SysGrade'>$CustWepS[complexity]</span>)";
echo "</td></tr><tr><td>";
...
echo "</select>點";
echo "</td></tr><tr><td>";
echo "武器名稱更換: <input type=text value=\"$Game[gamename]專用\" name=fixedname maxlength=32 onChange=\"ModName(this.value);\"><br>更換類型: <input type=radio name=namefix value=1 checked onclick=\"switchnfix('pre');\"> 字首附加 <input type=radio name=namefix value=2 onclick=\"switchnfix('suf');\"> 字尾附加";
