...
//
	...
	//Level Up
		...
		if ($Pl->Player['msexp'] >= $UserNextMsLvExp){$Pl->Player['mslv'] += 1;$Pl->Player['msexp'] = 0;$Result_Tag .="<br>你的機體升了級！";}
		if ($Op->Player['msexp'] >= $OppoNextMsLvExp){$Op->Player['mslv'] += 1;$Op->Player['msexp'] = 0;$Result_Tag .="<br>對手的機體升了級！";}
...
//
	...
	echo "<td>機體等級: ".$Pl->Player['mslv']." <br>主武器: ".$Pl->Eq['A']['name'].' <font style="font-size: 8pt; font-weight: 700;">('.$DisplayXp['Pl']['A'].'%)</font>'.getEqStatChange('Pl','A').'<br>';
	...
	echo "</td><td>機體等級: ".$Op->Player['mslv']." <br>主武器: ".$Op->Eq['A']['name'].' <font style="font-size: 8pt; font-weight: 700;">('.$DisOpayXp['Op']['A'].'%)</font>'.getEqStatChange('Op','A').'<br>';
	...
	echo "<td colspan=2>";
		...
		if($Pl->Player['level'] < 150) echo "得到 $Pl_Gain_Exp 點經驗值。<br>";
		if($Pl->Player['mslv'] < 30) echo "得到 $Pl_Gain_MsExp 點機體經驗值。<br>";
	...
	if ($Game_Scrn_Type == 0){
		if(!$mode){
			echo "<center><input type=button $BStyleB style=\"$BStyleA\" value=\"關閉\" onClick=\"parent.$SecTarget.location.replace('about:blank');parent.document.getElementById('STiF').style.left = -1150;\">";
		}else{
			echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
			echo "<center><input type=submit $BStyleB style=\"$BStyleA\" value=\"返回\" onClick=\"parent.Beta.location.replace('gen_info.php');\">";
		}
		//Requires Battle Continual System
		if($VictoryFlag == 0 || $VictoryFlag == 1) {
			echo "<input type=hidden name=\"actionb\" value=\"battle_sel\">";
			echo "<input type=button name=\"shortcutBattle\" $BStyleB style=\"$BStyleA\" disabled value=\"戰鬥(".$Btl_Intv.")\" onClick=\"movebattle();\"></center>";
		}
