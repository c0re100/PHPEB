...
if ($mod_hp_muiltiple > 10 || $mod_en_muiltiple > 10){echo "一次過最多只能改十次!!<br>";PostFooter();exit;}
...
elseif ($mode == 'repairms' && $actionb == 'reppro'){
	...
	if ($Game_Scrn_Type == 0) {
		...
		else echo " parent.document.getElementById('status_now').innerHTML='可戰鬥';parent.document.getElementById('status_now').style.color='#016CFE';";
...
elseif ($mode == 'modtypech' && $actionb == 'A'){
	...
	echo "<tr><td>";
	echo "<b>簡介:</b><br>人種改造是一個為類型(Type)為 一般(Natural) 的人，<br>改造身體，使自己成為其他人種。<br>";
	echo "<Br>但請緊記，一經改造，無法還原，亦無法再改造！";
	echo "</td></tr>";

	echo "<tr><td>";
	echo "<b>人種改造:</b><br>";
	echo "改造為:<br>";
	echo "<input type=radio name=dtype value=1 checked> 一般人<br><input type=radio name=dtype value=2> 強化人間<br><input type=radio name=dtype value=3> Extended<br><input type=radio name=dtype value=4> 念動力<br><input type=radio name=dtype value=5> New Type<br><input type=radio name=dtype value=6> CO<br>";
...
elseif ($mode == 'modtypech' && $actionb == 'B'){
	...
	switch($dtype){
		case 1: $Dest_Type = 'nat'; $ModChMsg = '一般人';break;
		case 2: $Dest_Type = 'enh'; $ModChMsg = '強化人間';break;
		case 3: $Dest_Type = 'ext'; $ModChMsg = 'Extended';break;
		case 4: $Dest_Type = 'psy'; $ModChMsg = '念動力';break;
