...
//組織指令
	...
	if ($Game['organization'] && $Game['rights']){
		if($Game['rights'] == '1'){
			$SC_Org .= "<br><input type=\"submit\" value=\"任命\" onClick=\"mainform.action='organization.php?action=Vice';actionb.value='A';\">";
...
//區域指令
	if ($Area["User"]["occupied"] == $Game['organization'] && $Game['rights']){
		$SC_Area .= "<input type=\"submit\" value=\"強化要塞\" onClick=\"mainform.action='city.php?action=ModFort';actionb.value='A';\">";
		$SC_Area .= "<input type=\"submit\" value=\"委派守衛\" onClick=\"mainform.action='city.php?action=AssignDefender';actionb.value='A';\">";
		$SC_Area .= "<br><input type=\"submit\" value=\"軍事力量投資\" onClick=\"mainform.action='city.php?action=Reinforcement';actionb.value='A';\">";
		$SC_Area .= "<input type=\"submit\" value=\"軍事調動\" onClick=\"mainform.action='city.php?action=Reinforcement';actionb.value='C';\">";
...
elseif ($mode=='settings' && $actionb == 'A'){
	...
	if (isset($ModifiedFlag)) echo "<br><font color=yellow>更新完成！</font>";
	echo "</b></td></tr>";
...
elseif ($mode == 'delete_account' && $actionb == 'B'){
	...
	echo "<br><br><br><br><br><p align=center style=\"font-size: 16pt\">已經刪除帳戶 <b style='color: red'>$_SESSION[username]</b> 。。。<br>";
	echo "<input type=button value=\"返回主頁\" onclick=\"location.replace('index.php');\">";
	echo "<br><br><br>";
}
//完結刪除帳戶系統
//開始兌換合金系統
elseif ($mode == 'redeemAlloy' && $actionb == 'A'){
	...
	echo "<tr><td align=left>";
	echo "您可以在這裡用勝利積分兌換 「新高達尼姆合金」 <br>兌換比率: <b>1 比 ".number_format($VPt2AlloyReq)."</b><Br>即是 ".number_format($VPt2AlloyReq)."點 勝利積分 可以換到 1 個合金！<br>";
	echo "所兌換的合金，會直接存到閣下的倉庫，請先確定倉庫有沒有多餘空間！<br>";
	echo "您的勝利積分: $Game[v_points]<br>";
	echo "<script language=\"JavaScript\">";
	echo "function vldPass(){";
		echo "if (document.getElementById('pwd').value != '$_SESSION[password]'){alert('密碼不正確。');return false;}";
		echo "else if (confirm('真的要兌換合金嗎？\\n您只會被問這一次，一旦兌換完成，合金不能還原成勝利積分，請考慮清楚！') == true) {return true;}";
		echo "else {return false;}";
	echo "}</script>";
...
//Process
	...
	if ($Countnumwhwp+$amountAlloy > 100){echo "武器庫空間不足。<br>請先清理一下，多謝合作！";exit;}
		...
		for($i=1;$i<=$amountAlloy;$i++) $Warehouse[1] .="\n".$AlloyEntry;
			...
			if($Game['v_points'] < 0){echo "勝利積分不足！";exit;}
			unset($sql);
		...
		echo "<p align=center style=\"font-size: 16pt\">兌換完成！<br><input type=submit value=\"返回\" onClick=\"parent.$SecTarget.location.replace('gen_info.php')\"></p>";
