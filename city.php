...
if ($mode=='ModFort'){
	...
	if ($actionb == 'A'){
		...
		if ($Area["User"]["hpmax"] + 10000 <= $Area["Sys"]["hpmax"]*10) echo "<br>強化要塞裝甲耐久度: <input type=submit value=\"增加10000HP\" onClick=\"return cfmModFort('hpmax');\"> 所需金錢: $". number_format($HpMax_Cost);
	
		if ($Otp_A_ITar) echo "<br>緊急維修: <input type=submit value=\"回復".$eRec."點HP\" onClick=\"return cfmModFort('ehp');\"> 所需金錢: $".number_format(25000);
		else{
