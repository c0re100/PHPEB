...
if ($mode=='main' && $actionb=='none'){
	...
	echo "if ($Gen[cash] < 150000){alert('您的所持金不足呢！請加油吧！');bkmainform.AccountValiding.style.visibility='visible';return false;}";
...
if ($actionb == 'AccValidation'){
	if (150000 > $Gen['cash']){echo "抱歉, 您的金錢不足。";postFooter();exit;}
...
elseif ($actionb == 'bounty'){
	...
	if($t_target == $_SESSION['username']){echo "麻煩閣下不要亂來。";postFooter();exit;}
...
mysql_query($sql) or die(mysql_error());unset($sql);
	...
	echo "<p align=center style=\"font-size: 16pt\">完成！<br><input type=submit value=\"返回\" onClick=\"parent.$SecTarget.location.replace('gen_info.php')\"><input type=button value=\"繼續使用銀行\" onClick=\"frmbk.submit()\"></p>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
...
elseif($mode == 'SafeHouse' && $actionb && $actionc){
	...
	elseif($actionb=='MakeDeal'){
		...
		if ($exch_target == $_SESSION['username']){echo "麻煩不要在這搗亂，多謝合作。";postFooter();exit;}
	...
	echo "<p align=center style=\"font-size: 16pt\">$Message<br><input type=submit value=\"返回\" onClick=\"parent.$SecTarget.location.replace('gen_info.php')\"><input type=submit value=\"繼續使用保險庫\" onClick=\"parent.$SecTarget.location.replace('bank.php?action=SafeHouse')\"></p>";
	echo "<input type=hidden name=\"TIMEAUTH\" value=\"$CFU_Time\">";
	echo "</form>";
