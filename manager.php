...
echo "<tr><td colspan=3><center><input type=radio name=operation value =1>用戶操作<input type=radio name=operation value =2>機體操作<input type=radio name=operation value =3>武器操作</center></td></tr>";
echo "<tr><td colspan=3><center><input type=radio name=operation value =4>合成操作<input type=radio name=operation value =5>批刪用戶<input type=radio name=operation value =6>批增機體</center></td></tr>";
echo "<tr><td colspan=3><center><input type=radio name=operation value =9>增加武器<input type=radio name=operation value =10>增加進化<input type=radio name=operation value =A>增加合成</center></td></tr>";
...
$oweplist = array("編號","名字","複雜性","等級","距離","屬性","可購買","攻擊","命中","回數","EN消耗","價格","可成輔助","特效","特殊");
...
if ("5" == $_POST["operation"] ) {
	...
	if(!$deletestatus &&!$deletetime && !$predel) {
		echo "<tr><td colspan=3><center><form action=manager.php?action=main method=post target=_self>";
		echo "<tr><td colspan=3><center>刪除條件。<br></center></td></tr>";
		echo "<tr><td colspan=3><center>帳戶狀態: <input type=text name='deletestatus' value = 2 size='20' maxlength=50></center></td></tr>";
	...
	if (!$num_rows){
		if($deletestatus){
			echo "<tr><td colspan=3><center>當前沒有狀態為 $deletestatus 的帳戶。<br></center></td></tr>";
		}
		exit;	
	} 
	echo "<table align=center width=250 border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"border-collapse: collapse;font-size: 10pt;\" bordercolor=\"#FFFFFF\">";
	...
	if (!$predel) {
		echo "<tr><td colspan=3><center>確定刪除以上帳戶？<br></center></td></tr>";	
		echo "<tr><td colspan=3><center><form action=manager.php?action=main method=post target=_self>";
		echo "<input type=hidden value='5' name=operation>";
...
if ("71" == $_POST["operation"]) {
	...
	while ( $i < $nq ) {
		...
		$sql = ("INSERT INTO ".$GLOBALS['DBPrefix']."phpeb_user_log (username,log1, time1) VALUES('$npcname','歡迎來到php-eb的世界！',UNIX_TIMESTAMP())");
...
if ("9" == $_POST["operation"] ) {
	...
	echo "<tr align=center><td width=100>複雜性</td>";
...
if ("101" == $_POST["operation"]) {
	...
	echo "<tr><td colspan=3><center>進化ID $evid 增加完成<br></center></td></tr>";                        
}
