...
if ($mode=='ms_custom' && $actionb == 'GUI'){
	...
	if(preg_match('/TransAM<([EnxNo]{2})><([0-9]+)>/',$Pl_Ms['spec'],$a)){
		if($a[1] != 'En') {
			echo "有 TransAM 系統的機體只能在正常狀態下進行改造！";
			exit;
		}
		unset($a);
	}
...
echo "&nbsp;　&nbsp;- 消耗 ".number_format($Mod_MS_vpt_cost)."點 勝利積分,<br>";
echo "&nbsp;　&nbsp;- 成功率下降 $Mod_MS_cpt_penalty%。<br>";
echo "&nbsp;- 每使用多一點「改造點數」, 成功率上升 $Mod_MS_cpt_bonus%。<br>";
echo "&nbsp;- 改造失敗的話, 原料(即改造點數)和機體都會毀掉, 勝利積分會保留。<br>";
echo "&nbsp;- 可以在兵器製造工場置放原料，轉化為改造點數。<br>您的改造點數: ".number_format($TactFactory['c_point'])."點 <br>你的勝利積分: ".number_format($Game['v_points'])."點 <br>您的現金: $".number_format($Gen['cash']);
echo "<hr></td></tr>";
...
echo "</td><td><b>改造成功率: </b><span id=successpc>$Mod_MS_base_success</span>%";
echo "</td></tr><tr>";
echo "<td width=50%>";
...
function confirmCustom(){
	if (pt_left.innerHTML > $TactFactory[c_point]){alert('改造點數不足！\\n無法進行改造。');return false;}
	else if (vp_left.innerHTML > $Game[v_points]){alert('勝利積分不足！\\n無法進行改造。');return false;}
	else if (custom_price.innerHTML > $Gen[cash]){alert('所持金不足！\\n無法進行改造。');return false;}
	else {
...
echo "</td></tr><tr>";
echo "<td>保險機制: <input type=checkbox name=secureCustom onClick=custom() value=true> (消耗 ".($Pl_Ms['needlv']*2)." 點改造點數)</td><td>消耗一定的改造點於失敗後保留機體。</td>";
...
if($Result_Success <= $SuccessPc){
	...
	$Result_Custom .= $ReF.'<!>';
	$Result_Custom .= $TaF;
	$Message = "成功改造了！<br>效果值: ".($Result_Success/100)."% < 成功率: ".($SuccessPc/100)."%";
	$MS_ResultG = "`v_points` = `v_points`-$vp_cost, ";
}
else {
	$Message = "改造失敗了。<br>效果值: ".($Result_Success/100)."% > 成功率: ".($SuccessPc/100)."%";
	$HP_Sub = $EN_Sub = 0;
	if(isset($a)) unset($a);
	...
	}else $Message .= "<br>工程師們成功修好損壞了的機體";
}
...
echo "<p align=center style=\"font-size: 16pt\">$Message<br><input type=submit value=\"返回\" onClick=\"parent.$SecTarget.location.replace('gen_info.php')\"></p>";
...
elseif ($Game['eqwep'] == '0<!>0' || !$Game['eqwep']){echo "請先裝備輔助裝備！";postFooter();exit;}
	...
	echo "> (消耗 ".($Pl_SyEqWep['complexity']*10 + $Pl_Ms['needlv']*2)." 點改造點數) ";
	echo "</td></tr>";
...
if (isset($secureCustom)){
	$secureCustom = 1;
	if (($Pl_SyEqWep['complexity']*10 + $Pl_Ms['needlv']*2) > $TactFactory['c_point']){echo "改造點數不足！";postFooter();exit;}
}
else $secureCustom = 0;
...
if($Result_Success <= $SuccessPc){
	...
	mysql_query($sql) or die(mysql_error());
	unset($sql);
	}else {$Message .= '<br>工程師們成功修好損壞了的裝備和機體。';$MS_ResultG = "`p_equip` = '0<!>0'";}
}
